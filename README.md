...
결제와 주문을 먼저 완료한 뒤 포인트 사용·적립, 예약 재고 확정, 알림, 정산 데이터를 처리합니다. 일부 단계가 실패해도 승인 결제를 되돌리지 않고 실패 내역을 남겨 영역별 복구 Job이 재처리합니다.
...
주문 생성, 결제 승인과 포인트 사용, PG 장애 처리, 환불·재고 동시성, Redis 이벤트와 실패 로그를 테스트합니다.
